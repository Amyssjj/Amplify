...
class GeneratedModelsValidationTests: XCTestCase {
    ...
    func testEnhancementRequestCodable() {
        ...
        let request = EnhancementRequest(
            photoBase64: testPhotoData.base64EncodedString(),
        ...
        XCTAssertNoThrow({
            ...
            XCTAssertEqual(decoded.photoBase64, testPhotoData.base64EncodedString())
    ...
    func testAllModelsCompile() {
        ...
        XCTAssertNoThrow({
            let _ = EnhancementRequest(
                photoBase64: Data().base64EncodedString(), transcript: "test")
            let _ = EnhancementTextResponse(
                enhancementId: "enh_test", enhancedTranscript: "test", insights: [:])
            let _ = EnhancementAudioResponse(audioBase64: Data(), audioFormat: .mp3)
            let _ = AuthResponse(
                accessToken: "token",
                user: AuthResponseUser(userId: "id", email: "test@example.com"))
