...
class MockAuthenticationService: AuthenticationServiceProtocol {
    ...
    func signInWithGoogle(idToken: String) async throws -> AuthResponse {
        ...
        // Check if we're set up to simulate a failure
        if case .error(let errorMessage) = mockAuthState {
            throw NSError(
                domain: "MockAuthenticationService", code: 401,
                userInfo: [NSLocalizedDescriptionKey: errorMessage])
        }

        // Use mock user if provided, otherwise use default
        let userToUse =
            mockUser ?? User(id: "mock_user_123", email: "test@example.com", name: "Mock User")

        let user = AuthResponseUser(
            userId: userToUse.id,
            email: userToUse.email,
            name: userToUse.name,
            picture: userToUse.profileImageURL
        )

        let response = AuthResponse(
            accessToken: mockToken ?? "mock_jwt_token",
            tokenType: .bearer,
            expiresIn: 3600,
            user: user
        )
...
class MockAudioRecordingService: AudioRecordingServiceProtocol {
    ...
    var mockStopResult: Result<AudioRecordingData, AudioRecordingError> = .success(
        AudioRecordingData(
...
class MockPhotoLibraryService: PhotoLibraryServiceProtocol {
    ...
    var mockFavoritesResult: Result<PHAssetCollection, PhotoLibraryError> = .failure(
        .permissionDenied)
    var mockPhotoResult: Result<PhotoData, PhotoLibraryError> = .success(
        PhotoData(
...
class MockSpeechRecognitionService: SpeechRecognitionServiceProtocol {
    ...
    func startLiveRecognition(onTranscriptUpdate: @escaping (String) -> Void) async -> Result<
