...
class APIClientTests: XCTestCase {
    ...
    func testAuthenticationWithInvalidToken() async {
        ...
        let errorResponse = ModelErrorResponse(error: "INVALID_TOKEN", message: "Invalid Google ID token")
    ...
    func testCreateEnhancementSuccess() async throws {
        ...
        let enhancementRequest = EnhancementRequest(
            photoBase64: "test_photo_data".data(using: .utf8)!,
        ...
        let expectedResponse = EnhancementTextResponse(
            ...
            enhancedTranscript: "This is my enhanced test story with better vocabulary and structure.",
            insights: [
        ...
        XCTAssertEqual(mockSession.lastRequest?.httpMethod, "POST")
        XCTAssertEqual(mockSession.lastRequest?.value(forHTTPHeaderField: "Authorization"), "Bearer valid_jwt_token")
    ...
    func testTokenRefreshOnUnauthorized() async throws {
        ...
        mockSession.responseQueue = [
            (try! JSONEncoder().encode(ModelErrorResponse(error: "UNAUTHORIZED", message: "Token expired")), unauthorizedResponse!),
            (try! JSONEncoder().encode(successResponse), HTTPURLResponse(url: URL(string: "http://localhost:8000/api/v1/enhancements")!, statusCode: 200, httpVersion: nil, headerFields: nil)!)
        ]
    ...
    func testRateLimitingWithRetry() async throws {
        ...
        mockSession.responseQueue = [
            (try! JSONEncoder().encode(ModelErrorResponse(error: "RATE_LIMITED", message: "Too many requests")), rateLimitResponse!),
            (try! JSONEncoder().encode(successResponse), HTTPURLResponse(url: URL(string: "http://localhost:8000/api/v1/enhancements")!, statusCode: 200, httpVersion: nil, headerFields: nil)!)
        ]
