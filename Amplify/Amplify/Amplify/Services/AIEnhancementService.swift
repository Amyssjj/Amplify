...
class AIEnhancementService: ObservableObject {
    ...
    private func performEnhancementRequest(transcript: String, duration: TimeInterval) async -> Result<StoryEnhancement, AIEnhancementError> {
    ...
    private func performInsightsRequest(transcript: String) async -> Result<[AIInsight], AIEnhancementError> {
    ...
    private func performStructureAnalysisRequest(transcript: String) async -> Result<StoryStructureAnalysis, AIEnhancementError> {
    ...
    private func performHighStakeWordsRequest(transcript: String) async -> Result<HighStakeWordsDetection, AIEnhancementError> {
    ...
    private func parseStructureAnalysisResponse(_ response: String) throws -> StoryStructureAnalysis {
        guard let data = response.data(using: .utf8),
              let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let analysisDict = json["structure_analysis"] as? [String: Any] else {
            throw AIEnhancementError.invalidResponse
        }
