...
class AudioPlayerService: NSObject, ObservableObject {
    ...
    func play() {
        ...
        if !success {
            ...
            print(
                "🔴 Playback failed - Player state: isPlaying=\(player.isPlaying), currentTime=\(player.currentTime)"
            )
            print(
                "🔴 Audio session state: category=\(AVAudioSession.sharedInstance().category), mode=\(AVAudioSession.sharedInstance().mode)"
            )
        }
    ...
    private func setupAudioPlayer(with audioData: Data) throws {
        ...
        print(
            "🔵 Audio format - Sample Rate: \(player.format.sampleRate), Channels: \(player.format.channelCount)"
        )
