...
struct EnhancementMapper {
    ...
    static func mapToRecording(
        ...
        print("🔵 MAPPER CALL [\(callId)]: mapToRecording(EnhancementTextResponse) for recording \(originalRecording.id)")
        ...
        return await MainActor.run {
            ...
            print("🔵 Recording had \(originalRecording.insights.count) insights, clearing and adding \(insights.count) new insights")
    ...
    static func mapToRecording(
        ...
        let callId = UUID().uuidString.prefix(8)
        print("🔵 MAPPER CALL [\(callId)]: mapToRecording(EnhancementDetails) for recording \(originalRecording.id)")
        ...
        return await MainActor.run {
            ...
            print("🔵 (Details) Recording had \(originalRecording.insights.count) insights, clearing and adding \(insights.count) new insights")
            originalRecording.insights.removeAll()
            for insight in insights {
                print("🔵 (Details) Adding insight: '\(insight.title)' (ID: \(insight.id))")
                originalRecording.addInsight(insight)
            }
            print("🔵 (Details) Recording now has \(originalRecording.insights.count) total insights")
    ...
    private static func determineCategoryFromContent(title: String, description: String) -> AIInsightCategory {
        let combinedText = "\(title) \(description)".lowercased()
        
        // Analyze content to determine the most appropriate category
        let emotionKeywords = ["feeling", "emotion", "vivid", "charm", "picture", "scene", "atmosphere", "mood"]
        let engagementKeywords = ["conversation", "reply", "response", "connect", "audience", "listener", "engaging", "chat", "two-way"]
        let techniqueKeywords = ["technique", "method", "approach", "way", "style", "manner", "conversational", "flow"]
        let clarityKeywords = ["clear", "understand", "explain", "detail", "specific", "precise", "clarity"]
        let vocabularyKeywords = ["word", "language", "phrase", "expression", "vocabulary", "wording"]
        let structureKeywords = ["structure", "organization", "format", "beginning", "middle", "end", "framework"]
        ...
        return scores.max(by: { $0.value < $1.value })?.key ?? .technique
    }
    ...
    private static func extractSuggestionFromDescription(_ description: String) -> String? {
        ...
        let suggestionIndicators = ["try", "consider", "use", "add", "start with", "instead of", "helps", "makes", "can"]
...
extension EnhancementMapper {
    ...
    static func mapToRecording(from summary: EnhancementSummary) -> Recording {
        let recording = Recording(
            ...
            timestamp: summary.createdAt ?? Date()
        )
